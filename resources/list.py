    ...
    if not 'id="page-LOGIN"' in content:
        if utility.get_setting('single_profile') == 'true' and 'id="page-ProfilesGate"' in content:
            ...
            if 'agid=' in url and match_api_root and match_api_base and match_identifier:
                ...
                add.directory(utility.get_string(30110), match_api_root[0] + match_api_base[0] + '/' +
                              match_identifier[0] + '/wigenre?genreId=' + genre_id +
                                 '&full=false&from=51&to=100&_retry=0', 'list_videos', '', video_type)
            elif match1:
                current_page = match1[0]
                next_page = str(int(current_page) + 1)
                add.directory(utility.get_string(30110), url.replace('&pn=' + current_page + '&', '&pn=' +
                                                                     next_page + '&'), 'list_videos', '', video_type)
            elif match2:
                current_from = match2[0]
                next_from = str(int(current_from) + 50)
                current_to = str(int(current_from) + 49)
                next_to = str(int(current_from) + 99)
                add.directory(utility.get_string(30110), url.replace('&from=' + current_from + '&',
...
def genres(type, video_type):
    xbmcplugin.addSortMethod(plugin_handle, xbmcplugin.SORT_METHOD_LABEL)
    if utility.get_setting('is_kid') == 'true':
        type = 'KidsAltGenre'
        content = utility.decode(connect.load_site(utility.kids_url))
        ...
        content = utility.decode(connect.load_site(utility.genres_url % (utility.get_setting('netflix_application'),
    ...
    for genre_id, title in unique_match:
        if not genre_id == '83':
            if utility.get_setting('is_kid') == 'true':
                add.directory(title, utility.main_url + '/' + type + '?agid=' + genre_id +
...
def tv_genres(video_type):
    xbmcplugin.addSortMethod(plugin_handle, xbmcplugin.SORT_METHOD_LABEL)
    content = utility.decode(connect.load_site(utility.main_url + '/WiGenre?agid=83'))
    ...
    for genre_id, title in match:
        add.directory(title, utility.main_url + '/WiGenre?agid=' + genre_id, 'listVideos', '', video_type)
    xbmcplugin.endOfDirectory(plugin_handle)
