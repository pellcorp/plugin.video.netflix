...
def list_video(video_id, title, thumb_url, is_episode, hide_movies, type, url):
    ...
    if match:
        ...
        desc = utility.unescape(description_temp)
    ...
    if match:
        rating = match[0]
    title = utility.unescape(title.decode('utf-8'))
    ...
    if '/my-list' in url and video_type_temp == type:
        ...
        adding.video_directory(title, video_id, next_mode, thumb_url, video_type, desc, duration, year, mpaa,
                               director, genre, rating)
        added = True
    return added
...
def list_genres(type, video_type):
    ...
    if utility.get_setting('is_kid') == 'true':
        type = 'KidsAltGenre'
        content = connection.load_site(utility.kids_url + '?locale=de-DE')
        print content
        ...
        content = connection.load_site(utility.genres_url % (utility.get_setting('netflix_application'),
    ...
    for genre_id, title in unique_match:
        if not genre_id == '83':
            if utility.get_setting('is_kid') == 'true':
                ...
                adding.directory(title, utility.main_url + '/browse/genre/' + genre_id + '?so=az', 'list_videos', '',
                                 video_type)
    xbmcplugin.endOfDirectory(plugin_handle)
...
def list_tv_genres(video_type):
    xbmcplugin.addSortMethod(plugin_handle, xbmcplugin.SORT_METHOD_LABEL)
    content = connection.load_site(utility.main_url + '/WiGenre?agid=83')
